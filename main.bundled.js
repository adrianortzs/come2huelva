...
export class Carousel {
  constructor({
    ...
    if (!this.track || !this.prevBtn || !this.nextBtn) {
      console.warn(`Carousel elements not found for ${trackSelector}`);
      return;
    }
...
export class ImageOptimizer {
  ...
  loadImage(img) {
    ...
    if (this.webpSupported && webpSrc && webpSrc !== originalSrc) {
      img.src = webpSrc;
    } else {
      img.src = originalSrc;
    }
...
export class LanguageManager {
  ...
  updatePageContent(t) {
    const updateSection = (containerSelector, headerKey, spansKey, isH1 = true) => {
      ...
      if (spans.length && t[spansKey]) {
        updateMultiple(spans, t[spansKey]);
      }
    };
  ...
  updateCTAButtons(t) {
    ...
    if (footerCta) {
      // Check which page we're on by checking the href attribute
...
export const translations = {
  es: {
    ...
    activitiesCards: [
      ...
      { title: "Astronomía", description: "Aprende a orientarte de noche y descubre los secretos del cielo mientras aprendes a usar un telescopio con tus propias manos. Podrás ver los cráteres de la luna y hacer tu primera foto astronómica con tu móvil a través del telescopio." },
  ...
  en: {
    ...
    introductionSpans: [
      ...
      "We want you to experience Huelva as we do.",
      "We invite you to discover an authentic Huelva with those who know and deeply love it.",
      "This is not a trip; it is an unforgettable experience that we want you to remember forever.",
      "DON'T BE A TOURIST.",
    ...
    placeSpans: [
      ...
      "We want you to discover the charm of Doñana, the fervor of El Rocío, the calm of endless beaches, and the depths of the earth in the Gruta de las Maravillas.",
      "Together we will show you each place as only we know it, feeling its essence and connecting with its history.",
    ...
    activitiesSpans: [
      ...
      "We want every experience to surprise you, to teach you something new and make it your own.",
    ...
    activitiesCards: [
      ...
      { title: "Astronomy", description: "Learn to orient yourself at night and discover the secrets of the sky while you learn to use a telescope with your own hands. You can see the craters of the moon and take your first astronomical photo with your mobile phone through the telescope." },
    ...
    gastronomySpans: [
      ...
      "Eating in Huelva is like making love. Every spoonful, a caress; every dish, a love story that will thrill your senses.",
      "With us, you won't just taste the food: you'll live it.",
    ...
    plansSpans: [
      ...
      "Our journeys are designed so you only have to worry about one thing: ENJOYING.",
      "We take care of EVERYTHING to make it unforgettable: transport, accommodation, meals, activities, experiences, and most importantly, accompanying you as local guides who know every corner and secret of this land.",
  ...
  fr: {
    ...
    introductionSpans: [
      ...
      "Nous vous invitons à découvrir une Huelva authentique avec ceux qui la connaissent et l'aiment profondément.",
      "Ce n'est pas un voyage ; c'est une expérience inoubliable que nous voulons que vous vous rappeliez pour toujours.",
    ...
    placeSpans: [
      ...
      "Nous voulons que vous découvriez le charme de Doñana, la ferveur du Rocío, le calme des plages infinies et les profondeurs de la terre dans la Grotte des Merveilles.",
      "Ensemble, nous vous montrerons chaque endroit comme nous seuls le connaissons, en ressentant son essence et en nous connectant à son histoire.",
    ...
    activitiesSpans: [
      ...
      "Nous voulons que chaque expérience vous surprenne, que vous appreniez quelque chose de nouveau et que vous l'intégriez comme la vôtre.",
    ...
    activitiesCards: [
      ...
      { title: "Astronomie", description: "Apprenez à vous orienter la nuit et découvrez les secrets du ciel tout en apprenant à utiliser un télescope de vos propres mains. Vous pourrez voir les cratères de la lune et prendre votre première photo astronomique avec votre téléphone portable à travers le télescope." },
    ...
    gastronomySpans: [
      ...
      "Manger à Huelva, c'est comme faire l'amour. Chaque bouchée, une caresse ; chaque plat, une histoire d'amour qui ravira vos sens.",
    ...
    plansSpans: [
      ...
      "Nous nous occupons de TOUT pour que ce soit inoubliable : transport, hébergement, repas, activités, expériences, et surtout, vous accompagner en tant que guides locaux qui connaissent chaque recoin et chaque secret de cette terre.",
...
// === main.js ===
import { CONFIG, $, $$, debounce, setExpanded, storage, showToast, validateEmail, validatePhone, throttle } from './js/utils.js';
import { translations } from './js/translations.js';
import { imageOptimizer } from './js/imageOptimization.js';
...
(function() {
  ...
  function scrollToIntroduction() {
    ...
    if (intro) {
      const introPosition = intro.getBoundingClientRect().top + window.pageYOffset;
      window.scrollTo({
  ...
  function changeLanguage(lang) {
    ...
    const translatedLinks = translations[lang].navLinks;
    navLinks.forEach((link, index) => {
      if (translatedLinks[index]) {
        link.textContent = translatedLinks[index];
      }
    });
    
    const introSpans = $$('.introduction-container span');
    const translatedIntroSpans = translations[lang].introductionSpans;
    introSpans.forEach((span, index) => {
      if (translatedIntroSpans[index]) {
        span.textContent = translatedIntroSpans[index];
      }
    });
    
    const placeHeader = $('#place-header');
    const placeSpans = $$('.place-container span');
    if (placeHeader) placeHeader.textContent = translations[lang].placeHeader;
    placeSpans.forEach((span, index) => {
      if (translations[lang].placeSpans[index]) {
        span.textContent = translations[lang].placeSpans[index];
      }
    });
    
    const activitiesHeader = $('#activities-header');
    const activitiesSpans = $$('.activities-container span');
    if (activitiesHeader) activitiesHeader.textContent = translations[lang].activitiesHeader;
    activitiesSpans.forEach((span, index) => {
      if (translations[lang].activitiesSpans[index]) {
        span.textContent = translations[lang].activitiesSpans[index];
      }
    });
    
    const gastronomyHeader = $('#gastronomy-header');
    const gastronomySpans = $$('.gastronomy-container span');
    if (gastronomyHeader) gastronomyHeader.textContent = translations[lang].gastronomyHeader;
    gastronomySpans.forEach((span, index) => {
      if (translations[lang].gastronomySpans[index]) {
        span.textContent = translations[lang].gastronomySpans[index];
      }
    });
    
    const plansHeader = $('#plans-header');
    const plansSpans = $$('.plans-container span');
    if (plansHeader) plansHeader.textContent = translations[lang].plansHeader;
    plansSpans.forEach((span, index) => {
      if (translations[lang].plansSpans[index]) {
        span.textContent = translations[lang].plansSpans[index];
      }
    });
    
    const opinionsHeader = $('#opinions-header');
    const opinionsSpans = $$('.opinions span');
    if (opinionsHeader) opinionsHeader.textContent = translations[lang].opinionsHeader;
    opinionsSpans.forEach((span, index) => {
      if (translations[lang].opinionsSpans && translations[lang].opinionsSpans[index]) {
        span.textContent = translations[lang].opinionsSpans[index];
      }
    });
    
    const formHeader = $('#form-header');
    const formSpans = $$('.form span');
    if (formHeader) formHeader.textContent = translations[lang].formHeader;
    formSpans.forEach((span, index) => {
      if (translations[lang].formSpans && translations[lang].formSpans[index]) {
        span.textContent = translations[lang].formSpans[index];
      }
    });
    ...
    if (nameLabel) nameLabel.textContent = translations[lang].formLabels.name;
    if (emailLabel) emailLabel.textContent = translations[lang].formLabels.email;
    if (phoneLabel) phoneLabel.textContent = translations[lang].formLabels.phone;
    if (messageLabel) messageLabel.textContent = translations[lang].formLabels.message;
    if (submitBtn) submitBtn.textContent = translations[lang].formLabels.submit;
    ...
    if (nameInput) nameInput.placeholder = translations[lang].formPlaceholders.name;
    if (emailInput) emailInput.placeholder = translations[lang].formPlaceholders.email;
    if (phoneInput) phoneInput.placeholder = translations[lang].formPlaceholders.phone;
    if (messageInput) messageInput.placeholder = translations[lang].formPlaceholders.message;
  }
  ...
  class Carousel {
    ...
    changeSlide(direction) {
      if (this.infinite) {
        ...
        if (this.currentIndex >= this.originalSlidesCount) {
          ...
          this.currentIndex = this.originalSlidesCount - 1;
          this.track.style.transition = 'none';
          this.updatePosition();
          this.track.offsetHeight;
          this.track.style.transition = 'transform 0.5s ease-in-out';
    ...
    pauseTemporarily() {
      ...
      this.resumeTimer = setTimeout(() => {
        this.startAuto();
      }, CONFIG.CAROUSEL.PAUSE_DURATION);
    }
  }
  ...
  function initFormSubmission() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!data.name || !data.email || !data.phone || !data.people || !data.message) {
        showToast(translations[currentLanguage].formValidation.required, 'error');
        return;
      }
      
      if (!validateEmail(data.email)) {
        showToast(translations[currentLanguage].formValidation.emailInvalid, 'error');
        return;
      }
      
      if (data.phone && !validatePhone(data.phone)) {
        showToast(translations[currentLanguage].formValidation.phoneInvalid, 'error');
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          showToast(translations[currentLanguage].formSuccess, 'success');
        form.reset();
        } else {
        ...
        console.error('Form submission error:', error);
        showToast(translations[currentLanguage].formError, 'error');
      } finally {
        LoadingOverlay.hide();
      }
    });
  ...
  function initVideoOverlay() {
    ...
    if (videoSrc) {
      const baseSrc = videoSrc.src.split('?')[0];
      videoSrc.src = `${baseSrc}?v=${currentTime}`;
      video.load();
    }
  ...
  window.scrollToForm = scrollToForm;
  window.scrollToIntroduction = scrollToIntroduction;
