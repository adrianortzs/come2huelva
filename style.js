...
function changeLanguage(lang) {
    ...
    if (languageButton) {
        if (lang === 'es') languageButton.innerHTML = 'Español &#9662;';
        else if (lang === 'en') languageButton.innerHTML = 'English &#9662;';
        else if (lang === 'fr') languageButton.innerHTML = 'Français &#9662;';
    }
    const langMenu = document.querySelector('.language-options');
    if (langMenu) langMenu.classList.remove('show');

    const introSpans = document.querySelectorAll('.introduction-container span');
    if (introSpans.length && t.introductionSpans) {
        t.introductionSpans.forEach((text, i) => {
            if (introSpans[i]) introSpans[i].textContent = text;
        });
    }

    const placeHeader = document.querySelector('.place-container h1');
    if (placeHeader && t.placeHeader) placeHeader.textContent = t.placeHeader;
    const placeSpans = document.querySelectorAll('.place-container > span');
    if (placeSpans.length && t.placeSpans) {
        t.placeSpans.forEach((text, i) => {
            if (placeSpans[i]) placeSpans[i].textContent = text;
        });
    }
    const placeOverlays = document.querySelectorAll('.places-container .place .overlay');
    translations[lang].placesOverlays.forEach((overlay, i) => {
    if (placeOverlays[i]) {
        const h3 = placeOverlays[i].querySelector('h3');
        const span = placeOverlays[i].querySelector('span');
        if (h3) h3.textContent = overlay.title;
        if (span) span.innerHTML = overlay.span;
    }
    });

    const activitiesHeader = document.querySelector('.activities-container h1');
    if (activitiesHeader && t.activitiesHeader) activitiesHeader.textContent = t.activitiesHeader;
    const activitiesSpans = document.querySelectorAll('.activities-container > span');
    if (activitiesSpans.length && t.activitiesSpans) {
        t.activitiesSpans.forEach((text, i) => {
            if (activitiesSpans[i]) activitiesSpans[i].textContent = text;
        });
    }
    const activityCards = document.querySelectorAll('.activity-card');
    if (activityCards.length && t.activitiesCards) {
        t.activitiesCards.forEach((card, i) => {
            if (activityCards[i]) {
                const h3 = activityCards[i].querySelector('h3');
                const span = activityCards[i].querySelector('span');
                if (h3) h3.textContent = card.title;
                if (span) span.textContent = card.description;
            }
        });
    }

    const gastronomyHeader = document.querySelector('.gastronomy-container h1');
    if (gastronomyHeader && t.gastronomyHeader) gastronomyHeader.textContent = t.gastronomyHeader;
    const gastronomySpans = document.querySelectorAll('.gastronomy-container > span');
    if (gastronomySpans.length && t.gastronomySpans) {
        t.gastronomySpans.forEach((text, i) => {
            if (gastronomySpans[i]) gastronomySpans[i].textContent = text;
        });
    }
    const gastronomyCards = document.querySelectorAll('.gastronomy-card');
    if (gastronomyCards.length && t.gastronomyCards) {
        t.gastronomyCards.forEach((card, i) => {
            if (gastronomyCards[i]) {
                const h3 = gastronomyCards[i].querySelector('h3');
                const span = gastronomyCards[i].querySelector('span');
                if (h3) h3.textContent = card.title;
                if (span) span.textContent = card.description;
            }
        });
    ...
    if (plansHeader && t.plansHeader) plansHeader.textContent = t.plansHeader;
    const plansSpans = document.querySelectorAll('.plans-container > span:not(.personalized-plan)');
    if (plansSpans.length && t.plansSpans) {
        t.plansSpans.forEach((text, i) => {
            if (plansSpans[i]) plansSpans[i].textContent = text;
        });
    }
    const planCards = document.querySelectorAll('.plan_card');
    if (planCards.length && t.plansCards) {
        t.plansCards.forEach((card, i) => {
            if (planCards[i]) {
                const headerSpan = planCards[i].querySelector('.card_header span');
                const contentSpan = planCards[i].querySelector('.card_content span');
                if (headerSpan) headerSpan.textContent = card.title;
                if (contentSpan) contentSpan.textContent = card.description;
            }
        });
    }
    const personalizedPlan = document.querySelector('.plans-container .personalized-plan');
    if (personalizedPlan && t.personalizedPlan) personalizedPlan.textContent = t.personalizedPlan;
    const personalizedPlanSpans = document.querySelectorAll('.plans-container .personalized-plan ~ span');
    if (personalizedPlanSpans.length && t.personalizedPlanSpans) {
        t.personalizedPlanSpans.forEach((text, i) => {
            if (personalizedPlanSpans[i]) personalizedPlanSpans[i].textContent = text;
        });
    }

    const opinionsHeader = document.querySelector('.opinions h1');
    if (opinionsHeader && t.opinionsHeader) opinionsHeader.textContent = t.opinionsHeader;

    const formHeader = document.querySelector('.form-container h1');
    if (formHeader && t.formHeader) formHeader.textContent = t.formHeader;
    const formSpans = document.querySelectorAll('.form-container span');
    if (formSpans.length && t.formSpans) {
        t.formSpans.forEach((text, i) => {
            if (formSpans[i]) formSpans[i].textContent = text;
        });
    }
    const labelName = document.querySelector('label[for="name"]');
    if (labelName && t.formLabels) labelName.textContent = t.formLabels.name;
    const labelEmail = document.querySelector('label[for="email"]');
    if (labelEmail && t.formLabels) labelEmail.textContent = t.formLabels.email;
    const labelTelf = document.querySelector('label[for="telf"]');
    if (labelTelf && t.formLabels) labelTelf.textContent = t.formLabels.phone;
    const labelPeople = document.querySelector('label[for="people"]');
    if (labelPeople && t.formLabels) labelPeople.textContent = t.formLabels.people;
    const labelMessage = document.querySelector('label[for="message"]');
    if (labelMessage && t.formLabels) labelMessage.textContent = t.formLabels.message;
    ...
    if (footerCta) {
        ...
        footerCta.textContent = isAboutPage ? t.footerCtaAbout : t.footerCtaIndex;
    }
    ...
    if (aboutUsSpans.length && t.aboutUsSpans) {
        t.aboutUsSpans.forEach((text, i) => {
            if (aboutUsSpans[i]) aboutUsSpans[i].textContent = text;
        });
    }
    ...
    if (ourSecretSpans.length && t.ourSecretSpans) {
        t.ourSecretSpans.forEach((text, i) => {
            if (ourSecretSpans[i]) ourSecretSpans[i].textContent = text;
        });
    }
}
